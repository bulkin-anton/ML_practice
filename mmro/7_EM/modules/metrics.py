...
def compute_precision(reference: List[LabeledAlignment], predicted: List[List[Tuple[int, int]]]) -> Tuple[int, int]:
    ...
    for i in range(len(reference)):
        a = set(predicted[i])
        intersect += len(a & (set(reference[i].sure) | set(reference[i].possible)))
        summ += len(a)
    return intersect, summ
...
def compute_recall(reference: List[LabeledAlignment], predicted: List[List[Tuple[int, int]]]) -> Tuple[int, int]:
    ...
    summ = 0
    for i in range(len(reference)):
        s = set(reference[i].sure)
        intersect += len((set(predicted[i])) & s)
        summ += len(s)
    return intersect, summ
...
def compute_aer(reference: List[LabeledAlignment], predicted: List[List[Tuple[int, int]]]) -> float:
    ...
    """
    p_n, p_d = compute_precision(reference, predicted)
    r_n, r_d = compute_recall(reference, predicted)
    if p_d + r_d == 0:
        return 0
    aer = 1 - ((p_n + r_n) / (p_d + r_d))
    return aer
